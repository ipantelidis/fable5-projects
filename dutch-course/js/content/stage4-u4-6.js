...
(function () {
  ...
  /* ---------------- Stage 4 exam (hand-written core in Staatsexamen style; the engine samples 30 per attempt) ---------------- */
  const exam = [
    mc('Which verb cluster is correct?', ['… dat het had moeten worden besproken.', '… dat het moeten had worden besproken.', '… dat het had worden moeten besproken.'], 0, 'Finite verb, modal, worden, participle.'),
    mc('Which sentence is correct?', ['Echter is de steun beperkt.', 'De steun is echter beperkt.', 'De steun echter is beperkt niet.'], 1, 'echter never opens the sentence.'),
    mc('"Ik kom niet; ik ben ___ ziek." (new information)', ['immers', 'namelijk', 'kortom'], 1, 'namelijk = a reason the listener did not know.'),
    mc('"We moeten opschieten; de trein wacht ___ niet." (shared knowledge)', ['immers', 'namelijk', 'echter'], 0, 'immers.'),
    mc('"Ik ga akkoord, ___ de reiskosten worden vergoed." (provided that)', ['tenzij', 'mits', 'doordat'], 1, 'mits.'),
    mc('"De trein viel uit ___ er een storing was." (cause, no intention)', ['omdat', 'doordat', 'zodat'], 1, 'doordat = cause.'),
    mc('The noun of "toenemen" is', ['de toename', 'de toeneming', 'het toegenomen'], 0, 'de toename.'),
    ...
    mc('"had not yet been investigated":', ['waren nog niet onderzocht', 'werden nog niet onderzocht', 'zijn nog niet onderzocht geworden'], 0, 'Pluperfect passive.'),
    mc('"Het verschil is niet te verklaren" means', ['it need not be explained', 'it cannot be explained', 'it was explained'], 1, 'zijn te + infinitive.'),
    mc('"De minister zou aftreden" in a news report means', ['the minister would like to resign', 'the minister is reported to be resigning', 'the minister resigned'], 1, 'Reporting zou = unconfirmed.'),
    mc('Toen ik aankwam, ___ de vergadering al begonnen.', ['is', 'was', 'had'], 1, 'beginnen takes zijn; pluperfect: was.'),
    ...
    mc('"Koste wat het kost" means', ['it costs what it costs', 'at all costs', 'for free'], 1, 'Fixed expression.'),
    mc('"Ik versta je niet" means', ['I do not agree with you', 'I cannot hear what you say', 'I do not like you'], 1, 'verstaan = catch the words.'),
    mc('A Flemish speaker says "seffens". In the Netherlands that is', ['gisteren', 'straks', 'nooit'], 1, 'seffens = straks.'),
    mc('"Dat mag zo zijn, maar …" is used to', ['add an argument', 'concede and then refute', 'conclude'], 1, 'Concession and refutation.'),
    mc('"Het roer omgooien" means', ['to change course radically', 'to sink the ship', 'to steer straight'], 0, 'Change of course.'),
    mc('"Eventueel" means', ['eventually', 'possibly / if need be', 'never'], 1, 'eventually = uiteindelijk.'),
    mc('Which is correct?', ['Het gebeurd vaak.', 'Het gebeurt vaak.', 'Het gebeurdt vaak.'], 1, 'Present tense: stem + t.'),
    mc('A writer who wants to "activeren" wants the reader to', ['laugh', 'do something', 'learn facts'], 1, 'activeren = call to action.'),
    fill('Dat had eerder ___ worden besproken. (should)', ['moeten'], 'That should have been discussed earlier.', 'Double infinitive inside the cluster.'),
    fill('___ het onderzoek blijkt dat de kosten stijgen.', ['Uit'], 'The study shows that costs are rising.', 'blijken uit.'),
    fill('Als ___ van de droogte mislukte de oogst.', ['gevolg'], 'As a result of the drought the harvest failed.', 'als gevolg van.'),
    fill('Nadat ik ___ afgestudeerd, ging ik solliciteren.', ['was'], 'After I had graduated, I went job hunting.', 'afstuderen takes zijn; pluperfect.'),
    fill('Tegen die tijd zal ik het rapport ___ afgerond.', ['hebben'], 'By then I will have finished the report.', 'Future perfect: zal … hebben afgerond.'),
    fill('Hoe meer je leest, ___ te groter wordt je woordenschat.', ['des'], 'The more you read, the bigger your vocabulary gets.', 'hoe … des te.'),
    fill('Hij spreekt geen Duits, laat ___ Fries.', ['staan'], 'He does not speak German, let alone Frisian.', 'laat staan.'),
    fill('Het ___ geen zin om te wachten.', ['heeft'], 'There is no point in waiting.', 'zin hebben, not maken.'),
    fill('Ik pleit ___ dat de regels eenvoudiger worden.', ['ervoor'], 'I argue that the rules should become simpler.', 'pleiten voor → ervoor dat.'),
    fill('Waar ___ "dit" naar in regel 12?', ['verwijst'], 'What does "dit" refer to in line 12?', 'verwijzen naar.'),
    build('The bill was passed with a small majority.', 'Het wetsvoorstel is met een kleine meerderheid aangenomen.', ['heeft', 'klein'], 'is … aangenomen.'),
    ...
    build('I should have negotiated earlier.', 'Ik had eerder moeten onderhandelen.', ['gemoeten', 'onderhandeld'], 'had moeten + infinitive.'),
    build('All in all the advantages outweigh the costs.', 'Al met al wegen de voordelen op tegen de kosten.', ['de voordelen wegen', 'opwegen'], 'Inversion; wegen … op tegen.'),
    build('I would advise you to contact the municipality.', 'Ik zou u aanraden om contact op te nemen met de gemeente.', ['opnemen te', 'raad'], 'contact op te nemen met.'),
    build('I overlooked that mistake.', 'Ik heb die fout over het hoofd gezien.', ['overzien', 'dat'], 'over het hoofd zien.'),
    fix('Namelijk ben ik ziek.', ['Ik ben namelijk ziek.'], 'namelijk never opens the sentence.'),
    fix('De resultaten zullen gepubliceerd geworden.', ['De resultaten zullen worden gepubliceerd.', 'De resultaten zullen gepubliceerd worden.'], 'Future passive.'),
    fix('Ik had eerder gemoeten overstappen.', ['Ik had eerder moeten overstappen.'], 'Double infinitive.'),
    fix('Hun zeggen dat het maakt geen zin.', ['Zij zeggen dat het geen zin heeft.', 'Ze zeggen dat het geen zin heeft.'], 'hun is never a subject; zin hebben; verb at the end.'),
    fix('De lange termijn planning word morgen besproken.', ['De langetermijnplanning wordt morgen besproken.'], 'One word; wordt.'),
    ...
    tr('nl-en', 'Beide partijen moesten water bij de wijn doen.', ['Both parties had to compromise.', 'Both sides had to compromise.'], 'Idiom.'),
    dict('Aangezien er geen meerderheid is, wordt het wetsvoorstel voorlopig ingetrokken.', 'Since there is no majority, the bill is being withdrawn for the time being.', 'aangezien … is; wordt … ingetrokken.'),
    ...
    speak('Volgens mij is thuiswerken een goede ontwikkeling, en wel om twee redenen.', 'In my view working from home is a good development, for two reasons.'),
    ...
    read('Opinie: Minder vliegen begint bij de prijs. Het aantal vluchten vanaf Schiphol is de afgelopen tien jaar sterk toegenomen, terwijl de trein naar Parijs of Berlijn vaak duurder is dan het vliegtuig. Dat is vreemd, want vliegen is immers veel schadelijker voor het klimaat. De oorzaak is bekend: op kerosine wordt geen belasting geheven, op treinkaartjes wel. Voorstanders van een vliegtaks wijzen erop dat de vervuiler hoort te betalen. Tegenstanders vrezen dat reizigers dan uitwijken naar luchthavens net over de grens. Dat mag zo zijn, maar dat pleit vooral voor een Europese aanpak, niet voor niets doen. Zolang de prijs de werkelijke kosten niet weerspiegelt, blijft elke oproep om vaker de trein te nemen zinloos. Kortom, wie wil dat mensen anders gaan reizen, moet eerst de prijzen eerlijk maken.', [
      { q: 'Wat is de hoofdgedachte van de tekst?', options: ['Schiphol moet sluiten.', 'Eerlijke prijzen zijn de voorwaarde voor ander reisgedrag.', 'Treinen zijn te langzaam.'], answer: 1, explain: 'The conclusion after "Kortom" restates the main idea.' },
      { q: 'Waarom gebruikt de schrijver "immers"?', options: ['om nieuwe informatie te geven', 'om de lezer te herinneren aan iets bekends', 'om een tegenstelling aan te geven'], answer: 1, explain: 'immers = shared knowledge.' },
      { q: 'Hoe gaat de schrijver om met het tegenargument?', options: ['hij negeert het', 'hij geeft het toe en weerlegt het daarna', 'hij verandert van standpunt'], answer: 1, explain: '"Dat mag zo zijn, maar …"' },
      { q: 'Wat is het doel van de schrijver?', options: ['informeren', 'overtuigen', 'amuseren'], answer: 1, explain: 'An opinion piece with a standpoint and arguments: overtuigen.' },
    ], { en: 'Opinion: flying less starts with the price. The number of flights from Schiphol has risen sharply over the past ten years, while the train to Paris or Berlin is often more expensive than the plane. That is strange, because flying is, after all, far more harmful to the climate. The cause is known: no tax is levied on kerosene, but it is on train tickets. Supporters of a flight tax point out that the polluter ought to pay. Opponents fear that travellers will then divert to airports just across the border. That may be so, but that mainly argues for a European approach, not for doing nothing. As long as the price does not reflect the real costs, every call to take the train more often remains pointless. In short, whoever wants people to travel differently must first make prices fair.' }),
  ];

  A.stage('s4', '4', 'B2', 'Upper intermediate', 'Dutch for study and professional life: long sentences, all past tenses, the full passive, formal and nominal style, argumentation, discourse markers, register switching, regional variation, proverbs, false friends and Staatsexamen NT2 Programma II training.', units, exam);
})();
