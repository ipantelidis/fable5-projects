...
- **Review of the skill (2026-09-23, from its README, not installed)**: MIT licensed. It ships a large design-intelligence dataset (192 industry rule sets, 192 palettes, 74 font pairings, 79 UI styles) searched by Python scripts (standard library only, no network), plus stack-specific guidance for 22 frameworks and a pre-delivery accessibility and resilience checklist. Install paths: `npm install -g ui-ux-pro-max-cli` + `uipro init --ai claude` (needs Node, not on this Mac), the Claude plugin marketplace (`/plugin marketplace add nextlevelbuilder/ui-ux-pro-max-skill`, interactive), or manual copy into `.claude/skills/`. Fit for this project: the palette and font generators are not needed (tokens already exist in css/app.css) and the framework guidance is aimed at React and Tailwind; the useful part is the accessibility and resilience checklist. Decision left to the owner: install it via the marketplace, or skip it and run the phase with the in-house checklist below. Either way the phase needs a real browser for visual checks.
- **Installed 2026-09-23** via `npx ui-ux-pro-max-cli@2.15.0 init --ai claude` into `.claude/skills/ui-ux-pro-max/` (SKILL.md, Python search scripts over CSV data, no network or shell use on inspection). The installer also added banner, brand, logo, slides, design-system and shadcn/Tailwind skills; those were removed as off-target. Use: `python3 .claude/skills/ui-ux-pro-max/scripts/search.py "<query>" --domain ux|style|typography|color` for guideline lookups, and its pre-delivery checklist before each screen is signed off.
